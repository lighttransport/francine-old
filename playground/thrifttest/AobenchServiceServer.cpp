...
class AobenchServiceHandler : public aobench::cpp2::AobenchServiceSvIf {
 public:
  AobenchServiceHandler() {
    // Your initialization goes here
  }

  void render(std::string& _return, const int32_t width, const int32_t height, const int32_t nsubsamples) {
    ...
    ::render(img, width, height, nsubsamples);
    _return = std::string(img, img + width * height * 3);
    delete[] img;
...
int main(int argc, char **argv) {
  ...
  auto handler = std::make_shared<AobenchServiceHandler>();
  ...
  server->setMaxRequests(5);
  server->setQueueSends(true);
